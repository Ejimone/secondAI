...
async def test_weather_functionality():
    ...
    for location in test_locations:
        ...
        try:
            ...
            logger.info(f"Status: {weather_response['status']}")
            logger.info(f"Type: {weather_response.get('type', 'N/A')}")
            logger.info("\nWeather Data:")
            logger.info(f"{weather_response.get('data', 'No weather data available')}")
            
        except Exception as e:
            logger.error(f"Error getting weather for '{location}': {str(e)}")
        
        logger.info("\nWaiting 2 seconds before next location...")
...
async def test_real_time_search():
    ...
    for query in test_queries:
        ...
        try:
            ...
            logger.info(f"Request Type: {response.get('request_type', 'N/A')}")
            logger.info(f"Data Type: {response.get('data_type', 'N/A')}")
            logger.info("\nSearch Result:")
            logger.info(f"{response.get('search_result', 'No results')}")
