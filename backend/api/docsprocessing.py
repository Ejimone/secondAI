...
from typing import Optional, Any, List, Dict, AsyncGenerator, Union
...
class RAGProcessor:
    ...
    async def stream_answer(self, query: str, k: int = 4) -> AsyncGenerator[Dict[str, Any], None]:
        """Stream answers in real-time with fallback to live search"""
        if not self.vector_store:
        ...
        try:
            ...
            docs = self.vector_store.similarity_search(query, k=k)
            if docs:
                yield {"type": "info", "content": "Found relevant documents..."}
                ...
                # Add sources
                yield {"type": "sources", "content": [doc.metadata for doc in docs]}
            
            else:
                # Fallback to live web search
                yield {"type": "info", "content": "No relevant documents found, searching web..."}
                async for result in self._stream_web_search(query):
                    yield {"type": "web_result", "content": result}

        except Exception as e:
            yield {"type": "error", "content": str(e)}

    async def _stream_web_search(self, query: str) -> AsyncGenerator[Dict[str, Any], None]:
        """Stream real-time web search results"""
        try:
            search = serpapi.GoogleSearch({
                "q": query,
                "api_key": os.getenv("SERPAPI_API_KEY")
            ...
            yield {"type": "error", "content": f"Web search failed: {str(e)}"}
...
async def main():
    try:
        ...
        if not urls and not pdf_paths:
            console.print("[red]No documents provided for processing[/red]")
            return
        ...
        # Interactive QA loop with streaming
        while True:
            ...
            with Live(console=console, refresh_per_second=4) as live:
                ...
                async for chunk in processor.stream_answer(question):
                    if chunk["type"] == "answer":
                        answer_text += chunk["content"]
                        live.update(Panel(answer_text, title="Answer"))
                    elif chunk["type"] == "sources":
                        sources = chunk["content"]
                    elif chunk["type"] == "web_result":
                        live.update(Panel(
                            f"[yellow]Web Result:[/yellow]\n{chunk['content']['title']}\n{chunk['content']['snippet']}",
                            title="Live Search"
                        ))
                    elif chunk["type"] == "info":
                        live.update(Panel(chunk["content"], title="Info"))
                    elif chunk["type"] == "error":
                        live.update(Panel(f"[red]{chunk['content']}[/red]", title="Error"))
