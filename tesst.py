...
async def real_time_search(user_prompt: str) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Received search query: {user_prompt}")  # Log the incoming query
        ...
        logger.info(f"Gemini Response: {response_text}")  # Log the response from Gemini
        ...
        if not all(key in request_info for key in ["type", "location"]):
            logger.error("Could not determine the type of information needed.")
            return {
                "status": "error",
                "message": "Could not determine the type of information needed."
            }
        
        # Route to appropriate handler based on request type
        if request_info["type"] == "weather":
            logger.info(f"Processing weather request for location: {request_info['location']}")
            query = f"current weather in {request_info['location']}"
            weather_summary = await scrape_webpages_with_serpapi(query)
            logger.info(f"Weather summary retrieved: {weather_summary}")  # Log the weather summary
            return {
                "status": "success",
...
async def scrape_webpages_with_serpapi(query: str) -> str:
    """Scrape weather information using SerpAPI"""
    ...
    try:
        logger.info(f"Searching for query: {query}")  # Log the search query
        params = {
        ...
        logger.info(f"Requesting SerpAPI with params: {params}")  # Log the request parameters
        search_response = requests.get("https://serpapi.com/search", params=params)
        logger.info(f"Response Status Code: {search_response.status_code}")  # Log the status code
        raw_results = search_response.json()
        
        logger.info(f"Raw search results: {raw_results}")  # Log the raw results
        
        if "error" in raw_results:
            logger.error(f"Search API error: {raw_results['error']}")
            return {
                "status": "error",
                "message": f"Search API error: {raw_results['error']}"
            }
        ...
        # Process the first result for weather information
        first_result = search_results[0]
        url = first_result.get("link", "")
        logger.info(f"Scraping URL: {url}")  # Log the URL being scraped
        
        # Scrape the content from the URL
        response = requests.get(url)
        if response.status_code == 200:
            # Here you can parse the response to extract weather information
            soup = BeautifulSoup(response.text, 'html.parser')
            weather_info = soup.find('div', class_='weather-info')  # Adjust this selector based on the actual HTML structure
            if weather_info:
                logger.info("Weather information found.")
                return weather_info.get_text(strip=True)
            else:
                logger.warning("Weather information not found in the scraped content.")
